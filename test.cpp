...
const float distance_scalar = 1.0f/831111.0f;
const float radius_scalar_sun= 1.0f/34817.0f;
const float radius_scalar_planet = 1.0f/631.07;
...
std::vector<PlanetRenderer> create_renderers(std::vector<Planet>& planet_data){
    ...
    for(const auto& p:planet_data){
        ...
        renderers.emplace_back(
            ...
            sf::Vector2f (0, p.distance_from_sun_km * distance_scalar),
            scaled_radius,
            //p.color,
            p.mass,
            sf::Vector2f (0, p.distance_from_sun_km),
            sf::Vector2f(-p.orbital_velocity, 0),
            p.planet_name
        );
    }
...
int main(){
    ...
    float dt = 60*60*24; //day in seonds

    //831111 km/px for distance
    PlanetLoader loader;
    std::vector<Planet> json_planets = loader.parse_planets(loader.load_from_file("planets_test.json"));
/*
